...
export default function OtpPage() {
  ...
  const handleVerify = () => {
    ...
    if (!/^\d{6}$/.test(otp)) {
      ...
      addToast('Code must contain only numbers', 'error');
      return;
    }
    setError('');
    setLoading(true);
  ...
  const handleResend = useCallback(() => {
    setTimerKey((k) => k + 1);
    addToast(`OTP resent to ${email || 'your email'}`, 'success');
  }, [email, addToast]);
      ...
      <div className="flex flex-col flex-1 min-h-0 justify-center">
        ...
        <div className="space-y-6">
          ...
          <div className="text-center">
            <TimerText key={timerKey} initialSeconds={28} onResend={handleResend} />
          </div>
