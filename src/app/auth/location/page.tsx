...
export default function LocationPage() {
  ...
  const handleComplete = () => {
    ...
    if (college.trim().length < 2) newErrors.college = college ? 'College name must be at least 2 characters' : 'College / Institution is required';

    if (Object.keys(newErrors).length > 0) {
      setErrors(newErrors);
      if (newErrors.state) addToast(newErrors.state, 'error');
      else if (newErrors.city) addToast(newErrors.city, 'error');
      else if (newErrors.college) addToast(newErrors.college, 'error');
      return;
    }
